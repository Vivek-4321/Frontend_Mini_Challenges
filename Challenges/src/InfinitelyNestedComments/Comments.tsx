    ...
    {comment?.items?.length > 0 && (
      <ul>
        {comment?.items.map(item => (
          <li style={{marginLeft: '30px', marginTop: '5px'}} key={Date.now()}>
            <Comments handleInsertNode={handleInsertNode} handleDeleteNode={handleDeleteNode} comment={item} />
          </li>
        ))}
      </ul>
    )}
