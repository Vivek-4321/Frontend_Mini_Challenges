...
const Calendar: React.FC = () => {
  ...
  const handleEventSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingEvent) {
      setEvents(events.map((event) => event.id === editingEvent.id ? { ...newEvent, id: event.id } : event));
    } else {
  ...
  const renderSidebarEvents = () => {
    const monthEvents = events.filter(
      (event) => event.date.getMonth() === currentDate.getMonth() && event.date.getFullYear() === currentDate.getFullYear()
    );
    ...
    return (
      <div className='sidebar-container'>
        <h3>{currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })} Events</h3>
        {monthEvents.sort((a, b) => a.date.getTime() - b.date.getTime()).map((event) => (
          <div
            key={event.id}
            className={`sidebar-event ${selectedDate?.toDateString() === event.date.toDateString() ? 'highlighted' : ''}`}
            onClick={() => {
    ...
    <div className="calendar-container">
      ...
      <div className={`event-form-overlay ${showEventForm ? 'active' : ''}`}>
        <form onSubmit={handleEventSubmit} className="event-form">
          ...
          {editingEvent && (
            <button type="button" onClick={handleEventDelete}>Delete Event</button>
          )}
